...
export default function ListSidebar({ apiUrl }: ListSidebarProps) {
    ...
    return (
        <aside className="z-50 sticky top-34 w-full hidden sm:block">
            <div className="border border-zinc-400 bg-white/90 p-3 shadow-sm dark:border-zinc-700 dark:bg-black/80">
                ...
                <p className="mb-3 font-mono text-[10px] uppercase leading-4 text-zinc-600 dark:text-zinc-300">
                Showing <span className="font-semibold text-zinc-900 dark:text-zinc-100">{selectedLabel}</span>.
                </p>

                <Calendar apiUrl={apiUrl} selectedCategory={selectedCategory} />

                <div className="w-full xl:container mx-0 xl:m-auto flex flex-col items-start justify-between gap-4">
                    {/* Filter */}
                    <div className="px-4 py-3 sm:py-0 w-full sm:w-fit flex items-center gap-2 border-b-1 sm:border-none border-zinc-300 dark:border-zinc-600">
                        <h4 className="block sm:block">Filter:</h4>
