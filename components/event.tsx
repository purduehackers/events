  ...
  <Link href="/[slug]" as={`/${slug}`} passHref>
    <a href="#">
      ...
      <div className={`col-span-1 shadow-lg flex flex-col rounded-lg justify-center p-5 sm:min-h-full ${past(end)
        ...
        <p className={past(end) ? '' : 'dark:text-black transition'}>
        ...
        <h1 className={`${name.length < 30 ? 'text-2xl' : 'text-2xl sm:text-xl'} ${past(end) ? '' : 'dark:text-black transition'} font-bold`}>{name}</h1>
      </div>
    </a>
  </Link>
)
